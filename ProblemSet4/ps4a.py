...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    word = ''
    total_score = 0
    while calculateHandlen(hand) > 0 and word != '.':
        print "Current Hand:", 
        displayHand(hand)
...
def playGame(wordList):
    ...
    startup = ''
    while startup != 'e':
        startup = (raw_input("Enter n to deal a new hand, r to replay the last hand, or e to end game: ")).lower()
        if startup == 'n':
            hand = dealHand(HAND_SIZE)
            games_played += 1
            playHand(hand.copy(), wordList, HAND_SIZE)
        elif startup == 'r' and games_played == 0:
